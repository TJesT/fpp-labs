...
#define EXIT(e) matrix_free(&A);matrix_free(&A_part);vector_free(&b);vector_free(&x);vector_free(&yn);return e
...
STATE scalarDotVectors(vector_t* a, vector_t* b, int* lines_in_task, int* shift_vector, int rank, double*   dst) {
    ...
    int     shift    = shift_vector[rank];
    double* a_part   = a->data + shift;
    double* b_part   = b->data + shift;
    for(uint64_t i = 0; i < N; ++i) {
        sum_part += a_part[i]*b_part[i];
    }
    MPI_Allreduce(&sum_part, dst, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
...
STATE multMatrixVector(matrix_t* A, vector_t* b, int* lines_in_task, int* shift_vector, int rank, vector_t* dst) {
    ...
    double* dst_part = dst->data + shift_vector[rank];
...
STATE substractVectors(vector_t* a, double a_k, vector_t* b, double b_k, int* lines_in_task, int* shift_vector, int rank, vector_t* dst) {
    ...
    double* a_part   = a->data   + shift;
    double* b_part   = b->data   + shift;
    double* dst_part = dst->data + shift;
    for(uint64_t i = 0; i < N; ++i) {
        dst_part[i] = a_k*a_part[i] - b_k*b_part[i];
    }
...
int main(int argc, char* argv[]) {
    ...
    do {
        CHANGE_STATE(state, calculateYn(&A_part, &x, &b, &yn, lines_in_task, shift_vector, rank));
        CHANGE_STATE(state, calculateTn(&yn, &A_part, &tn, lines_in_task, shift_vector, rank));
        CHANGE_STATE(state, calculateXnNext(&x, tn, &yn, lines_in_task, shift_vector, rank));
        MPI_Allgatherv(x.data, lines_in_task[rank], MPI_DOUBLE, tmp, lines_in_task, shift_vector, MPI_DOUBLE, MPI_COMM_WORLD);
        memcpy(x.data, tmp, sizeof(double) * N);
    ...
    if(rank == 0) {
        ...
        FILE* out = fopen("./data/result2-30-30.dat", "wb");
