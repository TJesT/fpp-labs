...
int main(int argc, char* argv[]) {
    ...
    int i = 0;
    do {
        if(calculateYn(&A, &x, &b, &yn) == MEMORY_ERROR) {
            matrix_free(&A);
            vector_free(&b);
            vector_free(&x);
            vector_free(&yn);
        ...
        if(calculateTn(&yn, &A, &tn) == MEMORY_ERROR) {
            matrix_free(&A);
            vector_free(&b);
            vector_free(&x);
            vector_free(&yn);
        ...
        if(calculateXnNext(&x, tn, &yn) == MEMORY_ERROR) {
            matrix_free(&A);
            vector_free(&b);
            vector_free(&x);
            vector_free(&yn);
    ...
    } while( i++ < ITERATION_COUNT && !hasSolution(&yn, &b) );
